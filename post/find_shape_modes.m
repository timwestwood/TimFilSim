...
% If false, the script will use the original shapes to generate modes.
% These can be made symmetric after the fact to still give axisymmetric
...
% Sometimes it fails to find a boundary properly and we end up with NaN
% everywhere, so throw away any image where this happened.
nan_ids = logical(sum(isnan(im_data), 2));
im_data_old = im_data; % Backup.
im_data = im_data_old(~nan_ids,:);
...
% Want to make the shape axisymmetric, but based on modes from the original
% data? Just use the same expression as in the make_symmetric_first toggle,
% but without the truncation. For example,
%
% im_avg(2:end) = 0.5*(im_avg(2:end) + fliplr(im_avg(2:end)));


% Need to save the modes to file (in the form expected by
% format_shape_fourier_modes.m and ultimately the code)?
%
% The example for the mean shape is
%
% im_avg_fourier = [im_avg_cos; im_avg_sin];
% im_avg_fourier = [max_num_wavenumbers, im_avg_fourier(:)'];
% dlmwrite('<INSERT NAME HERE>.fourier_modes', im_avg_fourier, ' ');
